...
// 매수픽 배열에 거래소 이벤트 방어 적용 — applyBuyModifiers를 안 쓰는 스캐너(momentum·flow)용.
// delist(mult 0)=픽에서 제외(상폐는 알림 안 함), halt/caution(mult<1)=score*=mult + pick.event 부착.
// 이벤트 없음=그대로. 알림(newEvents)은 monitor 전담(3중 스팸 방지) — 여기선 픽 방어만.
// 순수: picks 원본 불변, 새 배열 반환. 점수 바뀌므로 호출부가 재정렬해야 한다.
export function applyEventDefense(picks, byMarket = {}) {
  const out = []
  for (const p of picks || []) {
    const e = byMarket[p.market]
    if (!e || !(e.mult < 1)) { out.push(p); continue } // 이벤트 없음/무감점 → 그대로
    if (e.mult === 0) continue                          // 상폐 → 제외
    const score = typeof p.score === 'number' ? +(p.score * e.mult).toFixed(2) : p.score
    out.push({ ...p, score, event: { mult: e.mult, label: e.label } })
  }
  return out
}
...
// alerting: 이 호출이 "신규 이벤트 알림"의 주체인지. true(monitor)만 seenIds를 전진시키고 newEvents를
//   청구한다. false(momentum·flow)는 방어(active/byMarket)만 적용하고 알림 플래그는 건드리지 않는다 —
//   비알림 스캐너가 신규 플래그를 먼저 소비해 monitor 알림이 억제되는 순서 의존 버그 방지.
export async function ensureEvents(markets, { now = Date.now(), positions = [], deps = {}, alerting = true } = {}) {
  ...
  try {
    ...
    return await d.withLock('event-log', async () => {
      ...
      for (const c of sortedClassified) {
        ...
        // 알림 주체(monitor)만 신규 플래그를 소비/청구 — 비알림 스캐너는 방어만 적용하고 seenIds 불변.
        if (isNew && alerting) {
