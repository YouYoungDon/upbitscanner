    ...
    return await d.withLock('event-log', async () => {
      ...
      for (const c of sortedClassified) {
        ...
        for (const m of c.markets) {
          if (c.type === 'resume') {
            // 자기 ts 이하(과거)의 active만 지운다 — resume보다 나중(ts가 더 큰) halt/delist는 보존.
            // resume.ts가 불명(null, 예: 바이낸스 releaseDate 미노출)이면 "그보다 확실히 최신인
            // 이벤트를 지울 근거"가 없으므로 삭제를 건너뛴다(방어 우선: 새 halt를 오인 삭제하는
            // 쪽보다 안전 — 최악의 경우 만료가 늦게 청소될 뿐, 방어가 무력화되진 않는다).
            const resumeTs = effTs(c.ts)
            const arr = active[m]
            if (arr && resumeTs !== -Infinity) {
              const kept = arr.filter((e) => effTs(e.ts) > resumeTs)
      ...
      const coverage = markets && markets.length ? +(Object.keys(byMarket).length / markets.length).toFixed(2) : 0
