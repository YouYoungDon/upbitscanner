import { calcRSISeries, calcOBV, calcEMA, calcBBWidthSeries } from './indicators.mjs'
...
export function detectPumpStart(ohlcv, {
  ...
  // 밴드폭 시리즈를 한 번만 계산(중첩 루프 재계산 제거). bwAt(i)는 절대 인덱스 i의 BW.
  const bwSeries = calcBBWidthSeries(closes, bbPeriod, bbMult)
  const bwAt = (i) => (i >= bbPeriod - 1 ? bwSeries[i - (bbPeriod - 1)] : Infinity)
  const upperAt = (i) => {
  ...
  if (!(closes[last] > upperAt(last) && closes[last - 1] <= upperAt(last - 1) && volRatio >= volMult)) return null
  ...
  for (let t = last - 1; t >= last - sqWindow && t - sqLen >= 0; t--) {
    ...
    for (let k = t - sqLen; k < t; k++) if (k >= bbPeriod - 1) minPrior = Math.min(minPrior, bwAt(k))
