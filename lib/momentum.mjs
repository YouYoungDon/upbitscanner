...
// 백테스트 표본: 과거 각 시점에 scoreMomentum 적용 → MIN_SCORE 이상이면 forward return 기록.
export function backtestSamples(ohlcv, { minScore = MIN_MOMENTUM_SCORE, horizons = [3, 7], minWindow = 60 } = {}) {
  const out = []
  if (!Array.isArray(ohlcv)) return out
  const maxH = Math.max(...horizons)
  for (let i = minWindow; i < ohlcv.length - maxH; i++) {
    const { score } = scoreMomentum(ohlcv.slice(0, i + 1))
    if (score < minScore) continue
    const fwd = {}
    for (const h of horizons) fwd[h] = +(((ohlcv[i + h].close / ohlcv[i].close) - 1) * 100).toFixed(2)
    out.push({ index: i, score, fwd })
  }
  return out
}
