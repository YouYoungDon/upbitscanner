...
export const SIGNAL_KEYS = [
  ...
  '거래량 급증', '거래량 선행 매집', '쌍봉 패턴', '하락깃발 패턴', '역삼중바닥 패턴', '상승깃발 패턴', '상승삼각형 패턴',
  '박스권 돌파 패턴',
  '캔들 강세형', '캔들 약세형',
]
...
// 선행 매집 거래량 등급 (1.5/3/5/10x 계단). 06-28 실전: 예측력은 매집 거래량 크기에
// 거의 전적으로 의존(파워렛저 13x→+35% vs 맨틀 1.6x→무반응) → volR로 점수 차등.
export function accumulationGrade(volR) {
  if (volR == null || volR < 1.5) return 0
  if (volR < 3) return 1
  if (volR < 5) return 2
  if (volR < 10) return 3
  return 4
...
export function detectSignals(ohlcv, weights = {}) {
  ...
  const buy = [], sell = []
  const buyItems = [], sellItems = [] // 점수 합산 분해 {label, base, weight, score}
  let buyScore = 0, sellScore = 0
  const addBuy = (label, base) => {
    const weight = weightFor(weights, label), score = base * weight
    buy.push(label); buyItems.push({ label, base, weight, score }); buyScore += score
  }
  const addSell = (label, base) => {
    const weight = weightFor(weights, label), score = base * weight
    sell.push(label); sellItems.push({ label, base, weight, score }); sellScore += score
  }
  ...
  if (volR != null && volR >= 2 && volumes.length >= 2) {
    ...
    else if (down) { addSell(`거래량 급증 (${volR.toFixed(1)}x)`, grade); volSellFired = true }
  }
  // 거래량 선행 매집: 과매도 + 가격 횡보(-3%~+2%) + 거래량 미세증가(1.5x↑) → 폭발 전 단계 포착.
  // 위 거래량 급증(매수)은 +2% 상승 후에만 발동 → 그 전 매집 구간을 놓침. 매도 거래량 발생 시는 억제(모순 방지).
  const oversold = (stoch && stoch.k < 20) || (rsi != null && rsi < 30)
  if (volR != null && volR >= 1.5 && oversold && !volSellFired && volumes.length >= 2) {
    const prev = closes.at(-2)
    const chg = prev > 0 ? (closes.at(-1) - prev) / prev : 0
    if (chg > -0.03 && chg < 0.02) addBuy(`거래량 선행 매집 (${volR.toFixed(1)}x)`, accumulationGrade(volR))
  }
  ...
  return { buy, sell, buyItems, sellItems, buyScore, sellScore, price, volRatio: volR }
}

// 떨어지는 칼(falling knife) 필터: 과매도 골든크로스가 떴지만 거래량 미동반 +
// EMA 하락배열(추세 하락)이면 신뢰도 낮은 반등 → 매수 감점. 3박자 동시일 때만.
// 검증: 골든크로스 단독은 약함, 거래량 결합 시에만 강함(주간 저널). 거래량 없는 GC는
// 떨어지는 칼을 과매도라고 받는 그림.
export function fallingKnifePenalty(buySignals = [], sellSignals = []) {
  const hasGC = buySignals.some((s) => s.includes('골든크로스'))
  const hasVolume = buySignals.some((s) => s.startsWith('거래량 급증'))
  const emaBear = sellSignals.some((s) => s.startsWith('EMA 하락배열'))
  if (hasGC && !hasVolume && emaBear) {
    return { mult: 0.5, label: '[필터] 떨어지는칼(거래량無·하락배열)' }
  }
...
export function applyCombos(buy, sell, buyScore, volRatio = null) {
  let bs = buyScore
  const out = [...buy]
  const combos = [] // {label, mult} — 점수 합산 분해용
  ...
  if (!hasStochGC && hasRSI && hasBB && hasStoch && hasWR) {
    ...
    combos.push({ label: '[콤보] 과매도 함정 페널티', mult: 0.55 })
  }
  if (hasStochGC) {
    bs *= 1.4
    out.push('[콤보] 반등확인 보너스')
    combos.push({ label: '[콤보] 반등확인 보너스', mult: 1.4 })
  }
  if (hasVol) {
    const m = volComboMult(volRatio)
    bs *= m
    out.push('[콤보] 거래량확인 보너스')
    combos.push({ label: '[콤보] 거래량확인 보너스', mult: m })
  }
  return { buyScore: bs, buy: out, combos }
}
