...
async function cgGet(path, apiKey, { retries = 2, fetchImpl = fetch, sleepMs = 5000, timeoutMs = 10_000 } = {}) {
  for (let attempt = 0; ; attempt++) {
    try {
      const r = await fetchImpl(`${BASE}${path}`, { headers: { Accept: 'application/json', 'x-cg-demo-api-key': apiKey }, signal: AbortSignal.timeout(timeoutMs) })
...
// 전체 코인 id/symbol 목록 (심볼 매핑 재구축용, 주 1회). 본문이 수 MB라 기본 10s 타임아웃으론
// 느린 회선에서 매 시도 abort될 수 있어 30s로 늘린다(호출부가 opts로 재정의 가능).
export async function fetchCgCoinsList(apiKey, opts = {}) {
  if (!apiKey) return null
  return cgGet('/coins/list', apiKey, { timeoutMs: 30_000, ...opts })
}
