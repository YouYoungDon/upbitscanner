...
export function formatStrategy({ n, sl, tp, time, open, noData, winRate, avgRet, openList, risk }) {
  ...
  if (risk) lines.push(`📉 실현곡선: MDD ${pct(risk.mdd)} · 샤프 ${ratio(risk.sharpe)} (per-trade, n=${risk.n})`)
...
export function formatScorecard({ h1, h3, h7, total, pendingCount, risk }) {
  ...
  if (risk) lines.push(`📉 리스크(+1일곡선): MDD ${pct(risk.mdd)} · 샤프 ${ratio(risk.sharpe)} (n=${risk.n})`)
