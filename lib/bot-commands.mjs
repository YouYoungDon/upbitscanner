...
const ratio = (x) => (x == null ? '-' : Number(x).toFixed(2)) // 샤프 등 비율(연율화 없음)
const sym = (m) => m.replace('KRW-', '')

export function formatCoin({ korean_name, market, price, indicators: i, quietBottom, designation, pos90 }) {
  const shown = price ?? i.price // 실시간가 우선, 없으면 확정 종가
  const macdMark = i.macd == null ? '-' : i.macd.hist >= 0 ? '▲' : '▼'
  const lines = [`<b>${esc(korean_name)}</b> (${sym(market)}) · ${fmtP(shown)}원`]
  lines.push(`RSI ${i.rsi?.toFixed(1) ?? '-'} · Stoch K ${i.stoch?.k?.toFixed(1) ?? '-'} · MACD ${macdMark} · 거래량 ${i.volRatio?.toFixed(2) ?? '-'}배`)
...
export function formatStatus({ ratio, trend, buyCount, sellCount, topBuy, kimchi, funding }) {
  ...
  const kBand = { overheat: '과열🔴', discount: '디스카운트🔵', normal: '보통🟡' }[kimchi?.band] || ''
  const kLine = kimchi?.btcPremium != null
    ? `\n🇰🇷 김치프 ${kimchi.btcPremium >= 0 ? '+' : ''}${(kimchi.btcPremium * 100).toFixed(2)}% ${kBand}`
    : ''
  const fLine = funding?.medianRate != null
    ? `\n⚡ 시장 펀딩 ${funding.medianRate >= 0 ? '+' : ''}${(funding.medianRate * 100).toFixed(4)}%`
    : ''
  return `${emoji} <b>시장 요약</b>\n시장심리 ${ratio} · 매수 ${buyCount}/매도 ${sellCount}${kLine}${fLine}\n\n<b>상위 매수</b>\n${top}`
}

export function formatStrategy({ n, sl, tp, time, open, noData, winRate, avgRet, openList, risk }) {
  ...
  if (risk?.n >= 2) lines.push(`📉 실현곡선: MDD ${pct(risk.mdd)} · 샤프 ${ratio(risk.sharpe)} (per-trade, n=${risk.n})`)
...
export function formatScorecard({ h1, h3, h7, total, pendingCount, risk }) {
  const row = (label, h) => `${label}: ${h?.n ? Math.round(h.winRate * 100) + '%' : '-'} (평균 ${pct(h?.avgRet)}, 샤프 ${ratio(h?.sharpe)}, n=${h?.n ?? 0})`
  const lines = [`📊 <b>픽 스코어카드</b>`, row('+1일', h1), row('+3일', h3), row('+7일', h7)]
  if (risk?.n >= 2) lines.push(`📉 리스크(+1일곡선): MDD ${pct(risk.mdd)} · 샤프 ${ratio(risk.sharpe)} (n=${risk.n})`)
...
export function formatNotFound(query, suggestions) {
  const s = suggestions?.length ? `\n혹시 이거? ${suggestions.map(esc).join(', ')}` : ''
