...
// market 기준 교체 또는 추가. 기존 레코드가 있으면 검증된 필드를 그 위에 병합해
// openedAt 등 편집 범위 밖 필드를 보존. 새 배열 반환.
export function upsertPosition(list = [], position) {
  const existing = list.find((p) => p.market === position.market)
  const rest = list.filter((p) => p.market !== position.market)
  return [...rest, existing ? { ...existing, ...position } : position]
}
